...
def run_sweep(config: RunConfig) -> Tuple[Dict[str, Dict[float, np.ndarray]], Dict[str, Dict[float, np.ndarray]]]:
    ...
    t_stationary_start = int(math.floor(config.stationary_frac * config.tmax))
...
def write_scalar_tables(
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Criterio estacionario: t >= {int(math.floor(config.stationary_frac * config.tmax))}\n")
        f.write(
...
def plot_temporal_evolution(
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    if len(config.eta_values) < 3:
        chosen_etas = list(config.eta_values)
    else:
        chosen_etas = [config.eta_values[0], config.eta_values[len(config.eta_values) // 2], config.eta_values[-1]]
    ...
    axes[0].set_ylabel(r"Polarizacion $v_a(t)$")
    handles, labels = axes[-1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=min(4, len(labels)))
    fig.savefig(out_dir / "b_evolucion_temporal_va.png", dpi=220)
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--tmax", type=int, default=220, help="Total time steps per run")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="Start of stationary window as fraction of tmax (e.g., 0.6 => t>=0.6*tmax)",
    )
    ...
    config = RunConfig(
        ...
        stationary_frac=args.stationary_frac,
    ...
    plot_curves_c_and_d(out_dir, config, scalar_runs)
